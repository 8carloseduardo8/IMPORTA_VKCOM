...
public class Comunicador extends Integrador {
	...
	private final int canal = 27;
	private final String empresa = "0024";
	...
	public Comunicador(Finaliza f, ServletIntegradorNormal integrador) {
		super(f, integrador, "MABRA");
		con = Conector.getConexaoMabra();
	}
	...
	private void enviaPedido(Pedido pedido) throws Exception {
		...
		String sql = "SELECT * FROM UNISYS.TCADCLI0001 WHERE CL_CNPJ = " + Oracle.strInsert(pedido.cnpj);
		Resultado res = con.consultar(sql);
		if (!res.next()) {
			res.close();

			String mensagem = "";
			mensagem += "Atenção!" + System.lineSeparator();
			mensagem += "Pedido não processado por falta de cadastro do cliente na filial!" + System.lineSeparator();
			mensagem += "Segue abaixo informações do cliente:" + System.lineSeparator();
			mensagem += "CNPJ: " + pedido.cnpj + System.lineSeparator();
			mensagem += "RAZÃO: " + pedido.clienteRazao + System.lineSeparator();
			mensagem += "FANTASIA: " + pedido.clienteFantasia + System.lineSeparator();
			mensagem += "CODIGO VENDEDOR: " + pedido.setorVendedor + System.lineSeparator();
			mensagem += "VENDEDOR: " + pedido.vendedorNome + System.lineSeparator();

			// verifica se já foi enviado o emai para cadastro do cliente
			try {
				...
				if (rs.next() == false) {
					new Email("cifarma.manifesto@gmail.com", "cmcadmin").enviaSemAnexo(
							"CLIENTE NÃO CADASTRADO - CENTRAL DE PEDIDOS", mensagem,
					...
					// Email.enviaSemAnexo(
					// "CLIENTE NÃO CADASTRADO - CENTRAL DE PEDIDOS",
			...
			throw new Exception("CLIENTE NÃO CADASTRADO: " + pedido.cnpj);
		}
		res.close();
	...
	public void recebePedidos() throws Exception {
		...
		sql = "select p.ph_pedorigem, p.ph_origem,  n.nh_emissao ph_datafat, p.ph_numnota, p.ph_pedclie, c.cl_cnpj, v.* from pedido_a v ";
		...
		while (rs.next()) {
			try {
				...
				if (ped.status.equals("TRANSFERIDO") == false) {
					...
					// ATUALIZA O
					// STATUS
					Pedido pedAntes = pedDao.getPedido(ped.numero);
					if (pedAntes.status.equals(ped.status) == false) {
		...
		sql += "    select NUMERO from geral.pedido@dbl_cifgo ";
		...
		sql += "     select nh_numero from unisys.tnfhven0001@dbl_cifgo ";
	...
	public void recebeVendedores() throws Exception {
