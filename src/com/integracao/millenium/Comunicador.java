package com.integracao.millenium;
...
public class Comunicador extends Integrador {
	...
	private final int canal = 9;

	private FTPClient ftp = new FTPClient();

	public String host = "mll.net.br";
	public String usuario = "cifarma";
	public String senha = "*54f739";

	public static String pastaPedido = "/Cifarma/Cifarma/Entrada";
	public static String pastaNFe = "/Cifarma/Cifarma/NF";
	public static String pastaRetornoPedido = "/Cifarma/Cifarma/Retorno";
	public static String pastaEstoque = "/Cifarma/Cifarma/Estoque";
	...
	public File geraArquivoPedido(Pedido pedido) throws Exception {
		...
		// CNPJ MILLENIUM - 02632609000109
		// CNPJ JORGE B - 07222185000209

		File file = new File(
				"C:\\Atua\\PHL##07222185000209_" + new SimpleDateFormat("YYYYMMDDHHMMSS").format(new Date()) + ".txt");
	...
	public void recebePedidos() throws Exception {
		...
		for (FTPFile f : files) {
			...
			String arqDown = "C:\\VK-FARMA\\INTEGRACAO\\MILLENIUM\\" + f.getName();
	...
	public void recebeRetornoPedido() throws Exception {
		...
		FTPFile[] files = ftp.listFiles();
		System.out.println("ARQUIVOS A SEREM BAIXADOS: " + files.length);
		new File("C:\\VK-FARMA\\INTEGRACAO\\MILLENIUM\\RETORNOPEDIDO\\").mkdirs();

		for (FTPFile f : files) {
			if (f.isFile()) {
				System.out.println("DOWNLOAD NFE::: " + f.getName());
				String arqDown = "C:\\VK-FARMA\\INTEGRACAO\\MILLENIUM\\RETORNOPEDIDO\\" + f.getName();
				FileOutputStream fos = new FileOutputStream(arqDown);
				boolean download = ftp.retrieveFile(f.getName(), fos);
				if (download) {
					try {
	...
	public void recebeEstoque() throws Exception {
		...
		FTPFile[] files = ftp.listFiles();
		System.out.println("ARQUIVOS A SEREM BAIXADOS: " + files.length);
		new File("C:\\VK-FARMA\\INTEGRACAO\\MILLENIUM\\ESTOQUE\\").mkdirs();
		...
		for (FTPFile f : files) {
			if (f.isFile()) {
				...
				System.out.println("DOWNLOAD NFE::: " + f.getName());
				String arqDown = "C:\\VK-FARMA\\INTEGRACAO\\MILLENIUM\\ESTOQUE\\" + f.getName();
				FileOutputStream fos = new FileOutputStream(arqDown);
				boolean download = ftp.retrieveFile(f.getName(), fos);
				if (download) {
					try {
						...
						while ((s = br.readLine()) != null) {
							...
							if (s.startsWith("00")) {
								...
								estoque = Integer.parseInt(s.substring(16, 24));
