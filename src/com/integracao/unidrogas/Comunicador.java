package com.integracao.unidrogas;
...
public class Comunicador extends Integrador {
	...
	private final int canal = 2;
	private final String empresa = "0001";
	...
	public Comunicador(Finaliza f, ServletIntegradorNormal integrador) {
		super(f, integrador, "UNIDROGAS");
		con = Conector.getConexaoUnidrogas();
	}
			...
			try {
				...
				while (rsIt.next()) {
					...
					sql = "select * from unisys.tpdifaltas0001 where pi_numero = '" + rsIt.getString("PEDIDO")
							+ "' and pi_produto = '" + rsIt.getString("produto") + "'";
	...
	public void recebeTitulos() throws Exception {
		...
		sql += " SELECT MAX(C.CL_CNPJ) CNPJ, T.NUMERO, T.PARCELA, T.VENDEDOR, MAX(T.tipo), MAX(T.VENCIMENTO), SUM(T.VALOR) VALOR, T.STATUS, "
				+ canal;
		sql += " FROM UNISYS.TCADCLI0001@DBL_GO C, ";
		sql += " (SELECT * FROM TITULO_A@DBL_GO ";
		sql += " UNION ALL ";
		sql += " SELECT * FROM TITULO_B@DBL_GO ";
		sql += " UNION ALL ";
		sql += " SELECT * FROM TITULO_C@DBL_GO ";
		sql += " UNION ALL ";
		sql += " SELECT * FROM TITULO_D@DBL_GO ";
		sql += " ) T ";
	...
	public void recebeClientes() throws Exception {
		...
		while (res.next()) {
			try {
				...
				if (res.getString("ALVARAVALIDO").equals("SIM")) {
					cli.alvaraValido = Cliente.ALVARA_VALIDO;
				} else if (res.getString("ALVARAVALIDO").equals("NAO")) {
					cli.alvaraValido = Cliente.ALVARA_INVALIDO;
				} else {
					cli.alvaraValido = Cliente.ALVARA_VAZIO;
				}
	...
	public void enviaPedidos() throws Exception {
		...
		// .executar(
		// "UPDATE VEN_PEDIDO P SET STATUS = 'PENDENTE EXPORTAÇÃO', motivo =
		// 'LIBERADO AUTOMATICAMENTE PELA CENTRAL DE PEDIDOS', motivodata =
		// sysdate, motivousuario = 'CMC' WHERE STATUS = 'BLOQUEIO COMERCIAL'
		// AND CANAL = "
		// + canal);
	...
	public void recebeDevolucoes() throws Exception {
		...
		sql += " FROM UNISYS.TNFHCOM0001@DBL_GO H, UNISYS.TNFICOM0001@DBL_GO I, UNISYS.TCADCLI0001@DBL_GO C  ";
